...
解码输入预算覆盖排队任务及 MFT 已接收但未输出的画面，直到取得输出才释放。
有未完成输入时，解码 MTA 在空闲期间定时取输出；全部输出后取消轮询。
这处理软件 MFT 偶尔先返回 NEED_MORE_INPUT、稍后才有输出的情况，静止屏幕
无需等待下一包。轮询和提交、销毁串行，最旧输入仍受同一 5 秒预算约束。
...
开始和停止都保护整个资源所有权操作，包含原生线程返回后的句柄安装以及等待锁的停止任务。取消不会让后台线程继续使用已释放的句柄。正常停止先阻止音频提交，在解码器锁内解绑观察者，再排空原生编码/写入并结束 MP4；decoder 的释放在其后。macOS 录制视频最多保留 8 帧未完成任务（包含正在处理及排队的帧），为冷启动和短时性能波动留出缓冲；满时跳过新帧的编码提交，但仍更新独立的最新画面缓存用于补尾帧。AAC 最多 256 包 / 4 MiB，超限保存录制错误。原生提交必须有界，不能让文件写入反向阻塞解码器帧队列。原生队列/引用和具体编码配置见 control-flow.md。
