...
`quality` 默认为 `0.75`，接受 `0.0–1.0` 范围内的有限 int/float，不接受 bool、字符串或 None。类型错误抛 `TypeError`，越界或非有限数抛 `ValueError`，参数验证在创建文件之前完成。macOS 将参数传给 [VideoToolbox Quality](https://developer.apple.com/documentation/videotoolbox/kvtcompressionpropertykey_quality)；Windows 使用 Media Foundation 的质量 VBR，将参数映射到整数质量 1–100。越高通常画质越好、文件越大；不是固定码率、文件体积比例或跨平台、跨硬件一致的画质指标，`1.0` 不保证 H.264 无损。省略参数时使用 `0.75`。

macOS 视频从 NV12 直接经 VideoToolbox 硬件编码为 H.264，编码 Profile 和关键帧间隔采用系统默认设置，不指定固定码率，不启用牺牲画质的速度优先提示；保留实时编码、预期帧率和禁止帧重排配置。输出画布使用首帧尺寸；旋转后的 BT.709/未标记画面由 Metal 直接对 NV12 平面做双线性等比缩放、居中留黑；显式非 BT.709 输入保留基于 Metal 的色彩转换。录制使用部分 Metal 特性，推荐 Apple silicon，不保证 Intel Mac 能正常使用。开始时要求可用的 Metal 设备并完成 GPU pipeline 准备；Metal 初始化失败抛出 `RuntimeError`。

macOS 的硬件编解码路径面向常规屏幕流负载，录制视频最多保留 8 帧未完成任务（包含正在处理和排队的帧），用于吸收冷启动和短时性能波动。满时跳过新帧的编码提交，仍更新最新画面缓存用于补尾帧，原始 BGRA8/JPEG 路径继续可用。硬件加速不保证任何负载下都不过载；AAC 队列超限、编码失败或写入背压超时仍会报告录制错误。macOS 解码输入尚无显式的积压限额与过载检测。

Windows 使用 Media Foundation/SinkWriter 编码 H.264，通过系统视频处理器完成旋转后的等比缩放和居中留黑，画布固定为首帧尺寸。系统可以使用硬件加速，但不强制或验证最终使用的硬件。由于 Windows 硬件、驱动与实际编解码性能差异较大，容量或积压时间超限时持久报告 `MediaPipelineOverloadedError` 并停止相应管线，不通过丢帧继续运行性能不足的管线。解码错误关闭会话，录制错误只停止录制；具体容量和检测方式见 [Windows 文档](windows.md#线程容量和失败)。

两平台的实际码率和文件大小都随系统、尺寸和画面内容变化。停止会补齐缓存尾帧的持续时间，即使整段录制没有新视频包也能生成文件。
...
一包以内的音频时间戳抖动按连续采样时钟处理。macOS 超过一包的向前跳变保留静默区间；这类文件在停止时通过 macOS 原生接口修正 MP4 时间线，使用同目录临时副本并在成功后原子替换，不重新编码音视频，停止所需时间和临时磁盘空间会增加。macOS 时间戳向后重叠超过一包，或一份录制超过 4096 个间隔时报告错误。Windows 首版对超过一包的累计间隙或重叠抛出持久错误，不静默压缩时间轴，也不执行 macOS 的空编辑修复。
