...
软件 H.264 MFT 偶尔在输出尚未就绪时返回 `MF_E_TRANSFORM_NEED_MORE_INPUT`，
随后无需新输入即可取出画面。解码线程在仍有未完成输入时以 1 毫秒定时等待
检查输出，允许新输入继续入队；实际唤醒受系统调度影响。没有积压时停止轮询。
32 帧/64 MiB 预算同时覆盖排队和 MFT 内部尚未输出的输入，最旧输入超过
5 秒即持久报过载，不能无限等待或丢帧。轮询、提交和关闭都在同一 MTA 上执行。
...
| 解码尚未输出的输入（含排队及 MFT 内部） | 合计 32 帧 / 64 MiB 压缩数据 |
