...
该次运行首次尝试还出现一次 x64 普通 3.14 并发解码首帧等待超时；同一
提交重跑通过，本机 100 批、四线程、合计 1200 个解码器的对应压力测试也
未复现，尚未确定该次超时的原因。未修改解码器逻辑、放宽等待时间或跳过
该用例。原始失败记录保留在该 CI 的第一次尝试中。

本机普通 3.14.7 完整无设备回归为 **218 passed, 79 skipped**；
