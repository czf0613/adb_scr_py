...
## 2026-09-16 Windows 云端 CI 修复验证

首次 Windows CI 暴露三个问题：ARM64 3.14t 的裸版本请求选中了 x64
解释器；ARM64 普通版安装可选 MCP 的 cryptography 时缺少 OpenSSL 构建
环境；Windows Server 上冷启动后立即结束静止录制时，SinkWriter 接收了
两个视频样本但编码/处理计数仍为零，Finalize 报 `0xc00d4a44`。

修复为显式选择 Python 架构/ABI并向 uv 传递绝对路径，ARM64 与 3.14t
验证核心库，x64 普通版继续检查 MCP。录制收尾等待首个样本实际通过 sink，
受现有 5 秒积压预算约束；增加立即停止、1 毫秒、2 秒静止录制的重复回归。
异常包含收尾前的视频接收/编码/处理计数，保留原始失败和有界队列策略。

[修复分支 CI](https://github.com/czf0613/adb_scr_py/actions/runs/35050402634)
的五项矩阵已全部通过：x64 3.10/3.14/3.14t、原生 ARM64 3.14/3.14t。
均构建 sdist、从 sdist 构建 wheel、安装并导入原生扩展，再运行系统媒体
用例；3.14t 在启动、导入、测试后确认 GIL 关闭。这补充了 ARM64 的原生
云端编译和媒体运行验证，仍不等于 ARM64 手机连接或 Windows 10 实机验证。

主分支复验再次触发 x64 普通 3.14 并发解码首帧超时。限定四个 CPU、使用
系统内存路径后，本机成功复现：软件 MFT 先返回 NEED_MORE_INPUT，五秒后
直接再取输出即可得到首帧，无需新输入。增加解码 MTA 空闲轮询，输入预算
延伸至 MFT 产出画面；不扩大队列、延长超时或追加重复输入。修复后相同条件
四线程、100 批、1200 个解码器通过，并添加延迟输出、内部积压和超时回归。

macOS 26 / 3.14t 还暴露会话测试的计时竞争：模拟进程在 150 毫秒后自行
退出，慢机器上的重连可能尚未完成。改为连接成功后通过 stdin 明确触发
正常退出，仍验证断连通知和同一设备重连，不依赖固定进程寿命。

本机普通 3.14.7 完整无设备回归为 **221 passed, 79 skipped**；
已配置音频但尚未收到音频包即停止的补充检查也通过。未访问手机。
独立源树重建、重装 wheel 后，3.10.21 为 **183 passed, 2 skipped**，
3.14.7t 为 **184 passed, 1 skipped**；后者在启动、导入和测试后均确认
GIL 关闭。两者均检查 sdist/wheel 内容，真机脚本仅收集。
