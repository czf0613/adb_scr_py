...
连接排序、元数据等待、配置替换及取消所有权在两个平台共用。Windows
使用独立 MTA 工作线程驱动 Media Foundation，取代下文的 VideoToolbox/GCD；
图像转换使用系统视频处理器和 WIC，详见 [Windows 实现](windows.md)。
原始 BGRA8 通路仍为首要输出，固定无 padding，并直接写入最终 Python bytes。

Windows 容量/延迟超限使管线失败，不能用静默丢帧掩盖积压；独立状态监控
保证静止屏幕没有下一包时也能唤醒 `wait_media_error()`。正常 codec 缓冲
仍按 MFT 协议处理。录制持有最多八帧独立 NV12，复制后释放解码表面引用，
防止占满解码表面池。SinkWriter 禁用阻塞式默认限流，以样本数、字节数及
最旧样本 5 秒限制约束内部积压；连续 AAC 不再阻塞静止画面后的新视频。关闭等待
在途系统调用返回，5 秒错误预算不等于可强制销毁系统 codec。

Windows 起始画面使用 1 毫秒时长；1 微秒会在系统 MP4 timescale 中舍入成
零，导致重复 PTS。后续仍用下一帧/停止时间补 pending 帧，保持静止画面的
录制时长。AAC 超过一包的累计间隙/重叠在首版明确失败，不压缩时间轴。
持续 AAC 下，MP4 sink 不能无限等待静止画面的下一帧：当音频领先 pending
视频满 1 秒时补一段同画面视频，保留 500 ms 在途视频余量；晚于已提交区间
的视频报过载异常。这样系统内部 AAC 队列也能保持有界。
...
`action_series()` 对正的节点时长使用 ±10 ms 的随机等待目标，下界截到
1 ms，0 则跳过等待。2026-09-16 真机控制验证时修复了下界误用 `min`
的问题：旧实现会让 1–9 ms 节点触发延时参数断言，并可能把长节点的等待
大幅缩短。回归覆盖 1/5/10/1000 ms，检查实际延时函数接受参数并发出最终 UP。
...
录制器独立持有缓存画面。没有画面更新时不要求手机发送重复帧；停止时使用保留的尾帧补齐持续时间，并等待 VideoToolbox 和 MP4 writer 完成。编码队列最多保留 8 帧未完成任务（含正在处理的帧），为冷启动和短时性能波动留出缓冲。达到上限时跳过新帧的编码提交，仍更新最新画面缓存；不能静默丢弃音频后谎报成功。Metal 命令只在录制 GCD 队列内等待；输入、输出 pixel buffer 与 CVMetalTexture 引用保留到 GPU 完成，禁止向 VideoToolbox 提交尚未写完的 surface。初始化、映射或命令失败走录制错误处理。原生层在停止后拒绝新提交，观察者解绑、异步工作和最终资源释放都必须遵循引用所有权。
